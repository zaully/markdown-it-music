...
class Chord {
  constructor(root, quality, bass, duration) {
    this.root = root;
  ...
  aliases() {
    ...
    return rootAliases.flatMap((root) =>
      bassAliases.map((bass) => new Chord(root, this.quality, bass, this.duration)),
    );
  }
}
...
function parseChord(str) {
  ...
  return new Chord(tokens[2], tokens[3], tokens[4], tokens[1]);
}
